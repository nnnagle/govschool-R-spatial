...
This material was originally by Chester Ismay and Albert Kim for their book [https://moderndive.com](Statistical Inference via Data Science) and by Robin Lovelace and coauthors for their book [https://geocompr.robinlovelace.net](Geocomputation with R) . I have taken that material and turned it into an interactive tutorial.
...
Now consider the figure below which plots this data for all `r nrow(gapminder_2007)` countries in the data.
...
There is a lot of information in this visualization, but it's fairly intuitive and legible. 
On the two dimensional surface of the computer screen or paper, you see 4 different variables. 
Each country is represented by a point.
The x- and y-coordinates of the point represent the GDP per capita and Life Expectancy of the county. The color of the point represents the continent and the size represents the population.

In the next section, we'll learn to break down this chart into components that you can then mix and match to create a wide variety of different data visualizations.
...
The word *map* doesn't mean a cartographic map, but a mathematical map. A map assigns one thing (the data) to another thing (like color or size).
Example maps might be (percent below poverty) -> (color) or (latitude) -> (y- axis) or (longitude) -> (x- axis)
...
1. `aes`: aesthetic attributes of the geometric object. For example, x/y position, color, shape, and size.  Each assigned aesthetic attribute can be mapped to a variable in our data set. The word choice "aesthetic" always struck me as unusual, because I think of aesthetic as having a value, like "beautiful" or "ugly". But that's not the meaning here. The word "aesthetic" has an older meaning of "perceptible by the senses". In that regard, size, and color are perceptible by are senses, as is location along the x- and y-axes.
...
Below, you'll see `data` corresponds to the particular data frame where our data is saved and a "data variable" corresponds to a particular column in the data frame.
A data frame is just a rectangular table of data.
The type of `geom`etric object considered in this plot are points. While this plot just has points, other comment geometries include lines, polygons, and bars. 

For the plot on the previous page, let's summarize the three essential components of the Grammar in this Table .
...
Every chart has data, maps and aesthetics. They are like the nouns and verbs of sentences; every sentence has them.
There are other components of the Grammar of Graphics we can control as well. These are like the adjectives, prepositions and all the rest. As you start to delve deeper into the Grammar of Graphics, you'll start to encounter these topics more frequently. 
...
After we've specified these components, we then add *layers* to the plot using the `+` sign. The most essential layer to add to a plot is the layer that specifies which type of `geom`etric object we want the plot to involve: points, lines, bars, and others. Other layers we can add to a plot include layers specifying the plot title, axes labels, and visual themes for the plots.
...
The simplest of the 5NG are *scatterplots*, also called bivariate plots. They allow you to visualize the relationship between two numerical variables. While you may already be familiar with scatterplots, let's view them through the lens of the Grammar of Graphics. Specifically, we will use a dataset containing every passenger flight in or out of New York City during 2013. These data are provided for R users in the `flights` data frame included in the `nycflights13` package. We will visualize the relationship between the following two numerical variables

1. `dep_delay`: departure delay on the horizontal "x" axis and
1. `arr_delay`: arrival delay on the vertical "y" axis

```{r presetup, echo=FALSE, message=FALSE, warning=FALSE}
alaska_flights <- flights %>% 
  filter(carrier == "AS")
```

for all Alaska Airlines flights leaving NYC in 2013. This requires paring down the data from all 336,776 flights that left NYC in 2013, to only the `r nrow(alaska_flights)` *Alaska Airlines* flights that left NYC in 2013.

What this means computationally is: we'll take the `flights` data frame, extract only the `r nrow(alaska_flights)` rows corresponding to Alaska Airlines flights, and save this in a new data frame called `alaska_flights`. Run the code below to do this: 
...
```{r alaska-ex, exercise=TRUE}
alaska_flights <- flights %>% 
  filter(carrier == "AS")
```

Convince yourself that this code creates a new data frame `alaska_flights` consisting of only the `r nrow(alaska_flights)` Alaska Airlines flights.

This code takes the `flights` data frame and `filter`s it to only return the rows where `carrier` is equal to `"AS"`, Alaska Airlines' carrier code. Other examples of carrier codes include "AA" for American Airlines and "UA" for United Airlines. Recall from the tidyverse tutorial that a logical test for equality is specified with `==` and not `=`.



Take a look at both the `flights` and `alaska_flights` data frames by running `flights` and `alaska_flights`. In what respect do these data frames differ?
Try typing in `flights` and running the code. Repeat with `alaska_flights`.

```{r flights-view, exercise=TRUE}

```

### Scatterplots via geom_point {#geompoint}

Let's now go over the code that will create the desired scatterplot, keeping in mind our discussion on the Grammar of Graphics. We'll be using the `ggplot()` function included in the `ggplot2` package. 
...
* Within the `ggplot()` function, we specify two of the components of the Grammar of Graphics as arguments (i.e. inputs):
    1. The `data` frame is set to `alaska_flights` by setting `data = alaska_flights`.
    1. The `aes`thetic `mapping` by setting `mapping = aes(x = dep_delay, y = arr_delay)`. Specifically:
...
If you've taken a GIS class, then you will understand the idea of "layers" in a plot. 

After running the above code, you'll notice two outputs: a warning message and a figure. 

```{r noalpha, exercise=TRUE, fig.cap="Arrival Delays vs Departure Delays for Alaska Airlines flights from NYC in 2013", warning=TRUE, echo=FALSE}
ggplot(data = alaska_flights, mapping = aes(x = dep_delay, y = arr_delay)) + 
  geom_point()
```

Let's first unpack the warning message. The warning message alerts us to the fact that 5 rows were ignored due to them being missing. For 5 rows either the value for `dep_delay` or `arr_delay` or both were missing (R uses `NA` to respresent missing data), and thus these rows were ignored in our plot. Turning our attention to the resulting scatterplot in Figure \@ref(fig:noalpha), we see that a positive relationship exists between `dep_delay` and `arr_delay`: as departure delays increase, arrival delays tend to also increase.  We also note the large mass of points clustered near (0, 0).
...
* To stress the importance of adding layers, in particular the layer specifying the `geom`etric object, consider the following figure. We've set up the ggplot but didn't add any layers. It's not a very useful plot!


```{r nolayers, echo=TRUE, fig.cap="Plot with No Layers"}
ggplot(data = alaska_flights, mapping = aes(x = dep_delay, y = arr_delay))
```
...
**`r paste0("(LC ", (lc <- lc + 1), ")")`**  What are some practical reasons why `dep_delay` and `arr_delay` have a positive relationship?

**`r paste0("(LC ", (lc <- lc + 1), ")")`**  What variables (not necessarily in the `flights` data frame) would you expect to have a negative correlation (i.e. a negative relationship) with `dep_delay`? Why? Remember that we are focusing on numerical variables here.

**`r paste0("(LC ", (lc <- lc + 1), ")")`** Why do you believe there is a cluster of points near (0, 0)? What does (0, 0) correspond to in terms of the Alaskan flights?

**`r paste0("(LC ", (lc <- lc + 1), ")")`** What are some other features of the plot that stand out to you?

**`r paste0("(LC ", (lc <- lc + 1), ")")`** Create a new scatterplot using different variables in the `alaska_flights` data frame by modifying the example above, for example by mapping `sched_dep_time` to the x axis and `dep_delay` to the y axis.

```{r new-scatter, exercise=TRUE}
...
The large mass of points near (0, 0) in the previous Figure can cause some confusion as it is hard to tell the true number of points that are plotted.  This is the result of a phenomenon called *overplotting*.  As one may guess, this corresponds to values being plotted on top of each other _over_ and _over_ again.  It is often difficult to know just how many values are plotted in this way when looking at a basic scatterplot as we have here. There are two methods to address the issue of overplotting:
...
```{r jitter, exercise=TRUE, fig.cap="Jittered delay scatterplot"}
ggplot(data = alaska_flights, mapping = aes(x = dep_delay, y = arr_delay)) + 
...
**`r paste0("(LC ", (lc <- lc + 1), ")")`**  When is setting the `alpha` argument value useful with scatterplots? What further information does it give you that a regular scatterplot cannot?

**`r paste0("(LC ", (lc <- lc + 1), ")")`** Why is alpha not considered an aesthetic mapping? (Hint, is it an aesthetic? Is it a mapping?)

**`r paste0("(LC ", (lc <- lc + 1), ")")`** After viewing the Figure with alpha level, give an approximate range of arrival delays and departure delays that occur the most frequently.  How has that region changed compared to when you observed the same plot without the `alpha = 0.2`?
...
* Explore the `weather` data by running `(weather)`. 
...
```{r weather-explore, exercise=TRUE}
weather
```
...
```{r create-early-jan-weather, echo=TRUE}
early_january_weather <- weather %>% 
  filter(origin == "EWR" & month == 1 & day <= 15)
```

(Notice how I used '&' to filter on more than one thing. This is filtering on three different logical tests at once.)

**_Learning check_**


**`r paste0("(LC ", (lc <- lc + 1), ")")`** Take a look at both the `weather` and `early_january_weather` data frames by running `weather` and `early_january_weather`. In what respect do these data frames differ?


```{r lc-weather, exercise=TRUE}
...
```{r hourlytemp, exercise=TRUE, fig.cap="Hourly Temperature in Newark for January 1-15, 2013"}
ggplot(data = early_january_weather, mapping = aes(x = time_hour, y = temp)) +
  geom_line()
```

Much as with the `ggplot()` code that created the scatterplot of departure and arrival delays for Alaska Airlines flights, let's break down the above code piece-by-piece in terms of the Grammar of Graphics:
...
**`r paste0("(LC ", (lc <- lc + 1), ")")`**  Why should linegraphs be avoided when there is not a clear ordering of the horizontal axis?

**`r paste0("(LC ", (lc <- lc + 1), ")")`** Why are linegraphs frequently used when time is the explanatory variable on the x-axis?

**`r paste0("(LC ",  (lc <- lc + 1), ")")`** Plot a time series of a variable other than `temp` for Newark Airport in the first 15 days of January 2013.


```{r lc-weather2, exercise=TRUE}
ggplot(data = early_january_weather, mapping = aes(x = time_hour, y = ...)) +
  geom_line()
```
...
ggplot(data = weather, mapping = aes(x = temp, y = factor("A"))) +
  geom_point(alpha=.05, shape=19, stroke=0) +
...
```{r weather-histogram, exercise=TRUE, warning=TRUE, fig.cap="Histogram of hourly temperatures at three NYC airports."}
ggplot(data = weather, mapping = aes(x = temp)) +
  geom_histogram()
```
...
```{r weather-histogram-2, warning=FALSE, exercise=TRUE, message=FALSE, fig.cap="Histogram of hourly temperatures at three NYC airports with white borders."}
ggplot(data = weather, mapping = aes(x = temp)) +
...
```{r weather-histogram-3, exercise=TRUE, warning=FALSE, message=FALSE, fig.cap="Histogram of hourly temperatures at three NYC airports with white borders."}
ggplot(data = weather, mapping = aes(x = temp)) +
...
```{r hist1, warning=FALSE, exercise=TRUE, message=FALSE, fig.cap= "Histogram with 40 bins."}
ggplot(data = weather, mapping = aes(x = temp)) +
...
```{r hist2, warning=FALSE, exercise=TRUE, message=FALSE, fig.cap="Histogram with binwidth 10."}
ggplot(data = weather, mapping = aes(x = temp)) +
...
**`r paste0("(LC ", (lc <- lc + 1), ")")`** What does changing the number of bins from 30 to 40 tell us about the distribution of temperatures?

**`r paste0("(LC ",(lc <- lc + 1), ")")`** Would you classify the distribution of temperatures as symmetric or skewed?

**`r paste0("(LC ",  (lc <- lc + 1), ")")`** What would you guess is the "center" value in this distribution?  Why did you make that choice?

**`r paste0("(LC ", (lc <- lc + 1), ")")`** Is this data spread out greatly from the center or is it close?  Why?
...
```{r facethistogram, exercise=TRUE, echo=TRUE, fig.cap="Faceted histogram."}
ggplot(data = weather, mapping = aes(x = temp)) +
...
```{r facethistogram2, exercise=TRUE, fig.cap="Faceted histogram with 4 instead of 3 rows."}
ggplot(data = weather, mapping = aes(x = temp)) +
  geom_histogram(binwidth = 5, color = "white") +
...
**`r paste0("(LC ", (lc <- lc + 1), ")")`** What other things do you notice about the faceted plot above?  How does a faceted plot help us see relationships between two variables?

**`r paste0("(LC ", (lc <- lc + 1), ")")`** What do the numbers 1-12 correspond to in the plot above?  What about 25, 50, 75, 100?

**`r paste0("(LC ", (lc <- lc + 1), ")")`** For which types of data sets would these types of faceted plots not work well in comparing relationships between variables? Give an example describing the nature of these variables and other important characteristics.

**`r paste0("(LC ",(lc <- lc + 1), ")")`** Does the `temp` variable in the `weather` data set have a lot of variability?  Why do you say that?
...
While faceted histograms are one visualization that allows us to compare distributions of a numerical variable split by another variable, another visualization that achieves this same goal are *side-by-side boxplots*. A boxplot is constructed from the information provided in the *five-number summary* of a numerical variable. The five-number summary includes the: maximum, 75th percentile, median (50th percentile), 25th percentile, and the minimum,   To keep things simple for now, let's only consider hourly temperature recordings for the month of November.

This chart is not a boxplot, but it help to see where a boxplot comes from. Each data point is one dot (I've used alpha transparency to help with overplotting). The x-axis doesn't mean anything here, the dots are just randomly shifted left or right to further help with overplotting.
...
weather %>% 
  ...
  labs(x = " ", y="Temperature (degrees F)")
```
...
Let's mark these 5 values with dashed horizontal lines (behind the scenes, I just added another layer). 
...
Let's add the boxplot underneath these points and dashed horizontal lines.
...
What the boxplot does is: summarize the `r weather %>% filter(month == 11) %>% nrow()` points by emphasizing that:
...
Lastly, for clarity's sake let's remove the points but keep the dashed horizontal lines.
...
Let's now create a side-by-side boxplot of hourly temperatures split by the 12 months as we did above with the faceted histograms. We do this by mapping the `month` variable to the x-position aesthetic, the `temp` variable to the y-position aesthetic, and by adding a `geom_boxplot()` layer. Let's give it a go with month as the x aesthetic:

```{r badbox, exercise=TRUE, fig.cap="Invalid boxplot specification", fig.height=3.5}
...
That certainly didn't work. The warning messages clue us in as to why. The first warning message is telling us that we have a "continuous", or numerical variable, on the x aesthetic. Boxplots however require a categorical variable on the x-axis. 

We can convert the numerical variable `month` into a categorical variable by using the `factor()` function. So after applying `factor(month)`, month goes from having numerical values 1, 2, ..., 12 to having text (character) labels "1", "2", ..., "12." 

```{r monthtempbox, exercise=TRUE, fig.cap="Month by temp boxplot", fig.height=3.7}
ggplot(data = weather, mapping = aes(x = factor(month), y = temp)) +
  geom_boxplot() + 
  labs(x = "Month", y="Temperature (degrees F)")
```

The resulting Figureshows 12 separate "box and whiskers" plots with the features we saw earlier focusing only on November:
...
Depending on how your categorical data is represented, you'll need to add a different `geom` layer to your `ggplot()` to create a barplot, as we now explore. 
...
```{r geombar, echo=TRUE, fig.cap="Barplot when counts are not pre-counted", fig.height=2.5}
...
```{r, geomcol, echo=TRUE, fig.cap="Barplot when counts are pre-counted", fig.height=2.5}
ggplot(data = fruits_counted, mapping = aes(x = fruit, y = number)) +
  geom_col()
```

Compare the two barplots. They are identical because they reflect count of the same 5 fruit. However depending on how our data is saved, either pre-counted or not, we must add a different `geom` layer. When the categorical variable whose distribution you want to visualize is:
...
In order to create a barplot visualizing the distribution of the categorical variable `carrier` in this case, we would use `geom_col()` instead with `x` mapped to `carrier` and `y` mapped to `number` as seen below. The resulting barplot would be identical to the previous barplot.
...
**`r paste0("(LC ", (lc <- lc + 1), ")")`** Why are histograms inappropriate for visualizing categorical variables?

**`r paste0("(LC ", (lc <- lc + 1), ")")`** What is the difference between histograms and barplots?

**`r paste0("(LC ", (lc <- lc + 1), ")")`** How many Envoy Air (MQ) flights departed NYC in 2013?

**`r paste0("(LC ", (lc <- lc + 1), ")")`** What was the seventh highest airline in terms of departed flights from NYC in 2013? How could we better present the table to get this answer quickly?
...
Let's examine the same data used in our previous barplot of the number of flights departing NYC by airline, but this time we will use a pie chart:
...
While it is quite difficult to answer these questions when looking at the pie chart, we can much more easily answer these questions using the barchart. This is true since barplots present the information in a way such that comparisons between categories can be made with single horizontal lines, whereas pie charts present the information in a way such that comparisons between categories must be made by comparing angles. 
...
```{r flights-stacked-bar, echo=TRUE, fig.cap="Stacked barplot comparing the number of flights by carrier and origin.", fig.height=3.5}
ggplot(data = flights, mapping = aes(x = carrier, fill = origin)) +
  geom_bar()
```

This is an example of a *stacked barplot*.  While simple to make, stacked barplots can be challenging to interpret. For example, it is difficult to compare the heights of between the bars. For example, determine which carrier flies more out of Newark (EWR): American Airlines (AA), JetBlue (B6), or US Airways (US)?

Before we continue, let's address some common points of confusion amongst new R users. First, note that `fill` is another aesthetic mapping much like `x` or `y`; thus it must be included within the parentheses of the `aes()` mapping. The following code, where the `fill` aesthetic is specified outside the `aes()` mapping will yield an error. This is a fairly common error that new `ggplot` users make:
...
Second, the `fill` aesthetic corresponds to the color used to fill the bars, while the `color` aesthetic corresponds to the color of the outline of the bars. Observe in that mapping `origin` to `color` and not `fill` yields grey bars with different colored outlines. 

```{r flights-stacked-bar-color, echo=TRUE, fig.cap="Stacked barplot with color aesthetic used instead of fill.", fig.height=3.5}
...
**`r paste0("(LC ", (lc <- lc + 1), ")")`** What kinds of questions are not easily answered by looking at the above figure?

**`r paste0("(LC ", (lc <- lc + 1), ")")`** What can you say, if anything, about the relationship between airline and airport in NYC in 2013 in regards to the number of departing flights?
...
With this plot, it's a little easier to make comparisons, although the plot is more crowded. For example, if I again ask you determine which carrier flies more out of Newark (EWR): American Airlines (AA), JetBlue (B6), or US Airways (US), it's a little easier to answer. It's US Airways; it's bar a little taller than B6 or AA.

**_Learning check_**


**`r paste0("(LC ", (lc <- lc + 1), ")")`** Why might the side-by-side (AKA dodged) barplot be preferable to a stacked barplot in this case?

**`r paste0("(LC ",(lc <- lc + 1), ")")`** What are the disadvantages of using a side-by-side (AKA dodged) barplot, in general?


Lastly, another type of barplot is a *faceted barplot*. Recall in the Section on Faceting, we visualized the distribution of hourly temperatures at the 3 NYC airports *split* by month using facets. We apply the same principle to our barplot visualizing the frequency of `carrier` split by `origin`: instead of mapping `origin`
...
**`r paste0("(LC ", (lc <- lc + 1), ")")`** Why is the faceted barplot preferred to the side-by-side and stacked barplots in this case?

**`r paste0("(LC ",(lc <- lc + 1), ")")`** What information about the different carriers at different airports is more easily seen in the faceted barplot?
...
Let's recap all five of the Five Named Graphs (5NG) by summarizing their differences. Using these 5NG, you'll be able to visualize the distributions and relationships of variables contained in a wide array of datasets. This will be even more the case as we start to map more variables to more of each `geom`etric object's `aes`thetic attribute options, further unlocking the awesome power of the `ggplot2` package.

```{r viz-summary-table, echo=FALSE, message=FALSE, warning=FALSE}
...
In addition to the 5 basic plots, we can modify those plots further by adding faceting, alpha, dodging (or jittering) to make a wide variety of different visual styles. 


### Argument specification
We'll step away from the grammar of graphcs for a bit, and see how R handles arguments to functions. For example, the ggplot function takes two arguments, the data, and the mapping.

Run the following two segments of code. First this:

```{r arg1, eval = FALSE, exercise=TRUE}
ggplot(data = flights, mapping = aes(x = carrier)) +
  geom_bar()
```
...
```{r arg2, eval = FALSE, exercise=TRUE}
...
Going forward, all `ggplot()` will be like the second segment above: with the `data = ` and `mapping = ` being omitted. Everyone has a different preferences for how verbose their notes are.
