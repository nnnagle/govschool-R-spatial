...
To save a file to disk, so that you can insert it into a word document, for example, you use the `ggsave` function, like so:

```{r}
ggsave(filename = 'Black_Tennessee.png', height = 4, width=6)
```

This will save the last plot to a file.  Common file formats are '.png' and '.pdf'